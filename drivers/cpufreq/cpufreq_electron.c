...
static struct workqueue_struct *electron_wq;
...
static void __ref cpufreq_electron_event_boost(bool online) {
	unsigned int cpu;
	struct cpufreq_electron_policyinfo *ppol;

	if (online) {
		/* Put all cores online */
		for_each_possible_cpu(cpu) {
			if (cpu_online(cpu))
				continue;
			cpu_up(cpu);
		}
		/* Boost all cores to max frequency */
		for_each_possible_cpu(cpu) {
			ppol = per_cpu(polinfo, cpu);
			ppol->policy->cur = ppol->policy->max;
			ppol->policy->min = ppol->policy->max;
		}
		/* Unboost after a few seconds */
		queue_delayed_work(electron_wq, &unboost_work,
...
static void cpufreq_electron_unboost(struct work_struct *work)
{
	update_online_cpu_policy();
	event_boosted = false;
}

static void cpufreq_electron_suspend(void)
{
	pr_info("screen-off, turn off cores\n");
	cpufreq_electron_event_boost(false);
	event_boosted = false;
}

static void cpufreq_electron_resume(void)
{
	pr_info("recieved event, turn on cores and boost\n");
	cpufreq_electron_event_boost(true);
}


static int cpufreq_electron_state_notifier_call(struct notifier_block *this,
				unsigned long event, void *data)
{
	switch (event) {
		case STATE_NOTIFIER_ACTIVE:
		        if (unlikely(!event_boosted)) {
                		event_boosted = true;
				cpufreq_electron_resume();
			}
			break;
		case STATE_NOTIFIER_SUSPEND:
			if (unlikely(event_boosted))
				event_boosted = false;
			cpufreq_electron_suspend();
			break;
		default:
			break;
	}
...
static void cpufreq_electron_input_event(struct input_handle *handle,
...
static int cpufreq_electron_input_connect(struct input_handler *handler,
...
static void cpufreq_electron_input_disconnect(struct input_handle *handle)
{
...
static const struct input_device_id cpufreq_electron_ids[] = {
	{
...
static struct input_handler cpufreq_electron_input_handler = {
	.event          = cpufreq_electron_input_event,
	.connect        = cpufreq_electron_input_connect,
	.disconnect     = cpufreq_electron_input_disconnect,
	.name           = "electron_handler",
	.id_table       = cpufreq_electron_ids,
};
...
static int __init cpufreq_electron_init(void)
{
	...
	if (input_register_handler(&cpufreq_electron_input_handler))
		pr_info("%s input handler failed to register!\n", __FUNCTION__);

	/* Register state notifier */
	state_notifier_hook.notifier_call = cpufreq_electron_state_notifier_call;
	if (state_register_client(&state_notifier_hook))
		pr_info("%s state_notifier hook failed to register!\n", __FUNCTION__);

	INIT_DELAYED_WORK(&unboost_work, cpufreq_electron_unboost);

	electron_wq = alloc_workqueue("electron",
